 ...
 <div className="bg-white rounded-2xl p-4 shadow border">
  <div className="grid md:grid-cols-4 gap-4">
    ...
    <label className="block md:col-span-2">
     <span className="text-sm">التشخيص</span>
  ...
  <div className="mt-6">
   <div className="flex items-center justify-between mb-2">
    <h3 className="font-bold">الأدوية</h3>
    <button onClick={addMed} className="px-3 py-1 rounded-lg border">+ إضافة</button>
   </div>
   <div className="space-y-3">
    {meds.map((m, idx) => (
       <div key={idx} className="grid md:grid-cols-5 gap-3">
        <input placeholder="اسم الدواء" className="border rounded-xl p-3" value={m.name} onChange={e=>setMeds(update(meds, idx, { ...m, name:e.target.value}))} />
        <input placeholder="الجرعة" className="border rounded-xl p-3" value={m.dosage} onChange={e=>setMeds(update(meds, idx, { ...m, dosage:e.target.value}))} />
        <input placeholder="التكرار" className="border rounded-xl p-3" value={m.frequency} onChange={e=>setMeds(update(meds, idx, { ...m, frequency:e.target.value}))} />
        <input placeholder="المدة" className="border rounded-xl p-3" value={m.duration} onChange={e=>setMeds(update(meds, idx, { ...m, duration:e.target.value}))} />
        <div className="flex gap-2">
         <input placeholder="تعليمات" className="border rounded-xl p-3 flex-1" value={m.instructions} onChange={e=>setMeds(update(meds, idx, { ...m, instructions:e.target.value}))} />
         <button onClick={()=>removeMed(idx)} className="px-3 py-1 rounded-xl border">حذف</button>
        </div>
       </div>
    ))}
   </div>
   ...
   <div className="flex justify-end mt-6">
    <button onClick={submit} className="px-4 py-2 rounded-xl bg-indigo-600 text-white">حفظ الوصفة</button>
   </div>
 </div>
 );
}
