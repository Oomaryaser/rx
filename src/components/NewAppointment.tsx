...
export default function NewAppointment() {
 ...
 return (
  <div>
   <button onClick={()=>setOpen(true)} className="rounded-xl px-4 py-2 bg-indigo-600 text-white">+ زجح دعوم</button>
   {open && (
    <div className="fixed inset-0 bg-black/20 grid place-items-center p-4">
     <div className="w-full max-w-lg bg-white rounded-2xl p-6 space-y-3">
      <h3 className="text-lg font-bold">دعوم ديدج</h3>
      <label className="block">
       <span className="text-sm">خيراتلا</span>
      ...
      <div className="grid grid-cols-2 gap-3">
       <label className="block">
        <span className="text-sm">نم</span>
       ...
       <label className="block">
        <span className="text-sm">ىلإ</span>
      ...
      <label className="block">
       <span className="text-sm">ببس ةرايزلا</span>
      ...
      <div className="flex gap-2 justify-end pt-2">
       <button onClick={()=>setOpen(false)} className="px-4 py-2 rounded-xl border">ءاغلإ</button>
       <button onClick={submit} className="px-4 py-2 rounded-xl bg-indigo-600 text-white">ظفح</button>
      </div>
     </div>
    </div>
   )}
